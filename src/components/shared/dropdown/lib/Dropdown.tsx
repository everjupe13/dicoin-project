...
export function DropdownItem({
  ...
  const classes = clsx(
    className,
    // Base styles
    'rounded-lg focus:outline-hidden group cursor-default px-3.5 py-2.5 sm:px-3 sm:py-1.5',
    // Text styles
    ...
    // Forced colors mode
    'forced-colors:data-focus:bg-[Highlight] forced-colors:data-focus:text-[HighlightText] forced-colors:data-focus:*:data-[slot=icon]:text-[HighlightText] forced-color-adjust-none',
    ...
    'data-focus:*:data-[slot=icon]:text-white dark:data-focus:*:data-[slot=icon]:text-white *:data-[slot=icon]:text-zinc-500 dark:*:data-[slot=icon]:text-zinc-400',
    // Avatar
...
>) {
  return (
    <Headless.MenuSeparator
      ...
      className={clsx(
        className,
        'border-0 col-span-full mx-3.5 my-1 h-px bg-zinc-950/5 sm:mx-3 dark:bg-white/10 forced-colors:bg-[CanvasText]'
      )}
    />
  )
}
